...
def rewrite_contig_headers(header_lines, baits):
    ...
    inject ##INFO definitions for all LIFTOVER_* tags, and deduplicate scaffolds.

    The ##INFO lines MUST appear in the header before any variant record that
    uses those tags — bcftools will error on the first variant otherwise.
    """
    info_definitions = [
        '##INFO=<ID=LIFTOVER_BAIT,Number=1,Type=String,'
        'Description="Original bait sequence ID before liftover">\n',
        '##INFO=<ID=LIFTOVER_BAIT_POS,Number=1,Type=Integer,'
        'Description="Original 1-based position on bait sequence">\n',
        '##INFO=<ID=LIFTOVER_STRAND,Number=1,Type=String,'
        'Description="Alignment strand of bait to genome (plus or minus)">\n',
        '##INFO=<ID=LIFTOVER_GFFTYPE,Number=1,Type=String,'
        'Description="GFF feature type at lifted position (e.g. GLEANCDS or intergenic)">\n',
        '##INFO=<ID=LIFTOVER_PARENT,Number=1,Type=String,'
        'Description="Parent transcript ID from GFF attributes at lifted position">\n',
    ]
    ...
    for line in header_lines:
        # Inject INFO definitions just before the first existing ##INFO line,
        # or before #CHROM if the VCF has no ##INFO lines at all — but only once.
        if not info_injected and (line.startswith('##INFO=') or line.startswith('#CHROM')):
            new_headers.extend(info_definitions)
            info_injected = True
        ...
        if line.startswith('##contig='):
            ...
            if m:
                ...
                if bait_id in baits:
                    ...
                    continue  # drop the original bait-named contig line
        new_headers.append(line)

    # Guard: VCF had no ##INFO or #CHROM lines (degenerate case)
    if not info_injected:
        new_headers.extend(info_definitions)
...
def liftover_vcf(vcf_file, mapping_file, output_file, failed_file=None):
    ...
    with gzip.open(output_file, 'wt') as vcf_out:
        ...
        for line in variant_lines:
            ...
            # Sanitise free-text values before embedding in INFO:
            #   semicolons  -> '_'  (INFO field delimiter, would break parsing)
            #   spaces      -> '_'  (not allowed in INFO values)
            #   equals signs-> '_'  (reserved as key=value separator)
            def _safe(value):
            ...
            info = fields[7]
            provenance = (
                f"LIFTOVER_BAIT={_safe(bait_id)}"
                f";LIFTOVER_BAIT_POS={vcf_pos}"
                f";LIFTOVER_STRAND={_safe(lifted['strand'])}"
                f";LIFTOVER_GFFTYPE={_safe(lifted['gff_type'])}"
                f";LIFTOVER_PARENT={_safe(lifted['parent'])}"
            )
