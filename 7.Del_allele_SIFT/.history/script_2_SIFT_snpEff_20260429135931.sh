...
#
# Steps of the 5 scripts in this pipeline:
...
# Step 1 — Build snpEff database for Bmas
# Step 2 — Run SIFT4G scorer to build per-scaffold .regions files
# Step 3 — SIFT4G annotation
...
# Step 2 — Run SIFT4G scorer to build per-scaffold .regions files
#
# This step is only needed once. If .regions files already exist it is skipped.
# sift4g aligns proteins from your GFF against UniRef and writes one
# scaffold1.regions, scaffold2.regions … file per sequence into $SIFT_DB.
# These files are what SIFT4G_Annotator.jar looks up in Step 4.
#
# Flags:
#   -t   reference FASTA
#   -a   annotation GFF
#   --dbPath   path to UniRef90 database (FASTA, downloaded separately)
#   --outDir   output directory (same as $SIFT_DB used by annotator)
# ==============================================================================
UNIREF_DB="/home/tmichel/scratch/Deleterious_alleles_PNG_baits/7.Del_allele_SIFT/uniref90.fasta.gz"   # <-- edit this path

echo ""
REGIONS_COUNT=$(find "$SIFT_DB" -name "*.regions" 2>/dev/null | wc -l)
if [ "$REGIONS_COUNT" -gt 0 ]; then
    echo "[3b/5] SIFT4G .regions files already exist ($REGIONS_COUNT found) — skipping scorer."
else
    echo "[3b/5] Running SIFT4G scorer — this may take several hours..."
    ...
    if [ ! -f "$UNIREF_DB" ]; then
        ...
        echo "    gunzip uniref90.fasta.gz" >&2
        exit 1
    fi
        ...
        gffread "$BMAS_GFF" \
        -g "$BMAS_FASTA" \
        -y "$OUTPUT_DIR/Bmas_proteins.fa"
        # Quick sanity check — should show sequences starting with M (methionine)
        grep -c '^>' "$OUTPUT_DIR/Bmas_proteins.fa"
        head -4 "$OUTPUT_DIR/Bmas_proteins.fa"

        # Create a named pipe and feed the decompressed DB through it
        ...
        if [ ! -f "$UNIREF_UNZIPPED" ]; then
          echo "    Decompressing UniRef90 to scratch — this may take a while..."
          gunzip -c "$UNIREF_DB" > "$UNIREF_UNZIPPED"
        fi

        sift4g \
        -q "$OUTPUT_DIR/Bmas_proteins.fa" \
        -d "$UNIREF_UNZIPPED" \
        --out "$SIFT_DB" \
        -t 8
        SIFT4G_EXIT=$?
...
head -4 "$OUTPUT_DIR/Bmas_proteins.fa"
    ...
    echo "[!] SIFT4G scorer failed." >&2
    exit 1
    fi

    REGIONS_COUNT=$(find "$SIFT_DB" -name "*.regions" 2>/dev/null | wc -l)
    echo "    SIFT4G scorer complete — $REGIONS_COUNT .regions files written to $SIFT_DB"
fi
...
# SIFT4G_Annotator does not accept bgzipped input — decompress first
SORTED_VCF_PLAIN="$OUTPUT_DIR/all_renamed.genome_coords.sorted.vcf"
echo "    Decompressing VCF for SIFT4G..."
...
# Clean up the uncompressed copy to save space
