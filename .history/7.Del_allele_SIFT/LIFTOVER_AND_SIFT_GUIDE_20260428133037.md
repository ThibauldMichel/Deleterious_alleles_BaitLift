...
1. SIFT4G native tools
   SIFT4G_Create_Genomic_DB (build species database) -> SIFT4G_Annotator.jar (annotate VCF from the database)
2. SIFT4G via snpEff
   SIFT4G_Annotator.jar (same than before, but pointed at snpEff-format) -> snpEff.jar (separate tool that add functional annotations: gene names, impact, etc..)

snpEff provides richer annotations, so we will continue to work with this pipeline (transcript consequences, gene IDs, splice effects).
